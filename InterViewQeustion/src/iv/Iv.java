...
public class Iv {
    ...
    public static void method1() {                                                          // check using split method ( coma and double quotes)
          try{  
            ...
            LinkedList<String> arr1 = new LinkedList();                                     // to store each line from the text file .
                    ...
                    if(k==0 || k==2){                                                       
                        ...
                        for (int j=0;j<split1.length;j++){
                            try {                                                            // method to found age
                                ...
                                newAr.add(s);                                               //  add to linked list
            ...
            for(int i=1;i<arr.length;i++){
                for(int j=arr[i].length()-1;j>=0;j--){
                    try{                                                                                            // method to found age using hardcode 
                        ...
                        if(arr[i].charAt(j-1) == ',' && stack.size()==1){                                           // if found coma and stack is not empty it will break so that it will not check other integer value which is not age.
                            break;
                        } else {
                            int s1 = Integer.parseInt(arr[i].charAt(j-1)+"");                                       // else if not found coma means that the age is 2 digit. so it will put in stack.
                            stack.add(s1+"");
                            if(arr[i].charAt(j-2) == ',' && stack.size()==2){                                       //if found coma and stack is not empty it will break so that it will not check other integer value which is not age.
                                break;
                            } else {
                                int s2 = Integer.parseInt(arr[i].charAt(j-2)+"");                                   // else if not found coma means that the age is 3 digit. so it will put in stack.
                                stack.add(s2+"");
                                break;                                                                              
                            }
                        }
